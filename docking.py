...
def process(molpdb, r_num, l_num, xp_num):
    ...
    try:
        ...
        while recept_num <= r_num:
            ligand_file = 'data/'+str(xp_num)+'/vina_files/ligand_'+str(l_num)+'-r_'+str(recept_num)+'.pdbqt'
...
def moldocking(xp_num, num_mols, num_recept, path_to_py_scripts, start_ligand):
    ...
    while ligand_num <= num_mols:
        ...
        ligand_pdb = os.path.join(data_path, 'output_mols', str("ligand_"+str(ligand_num)+".pdb"))
        ...
        try:
            os.system("pythonsh "+str(path_to_py_scripts)+"prepare_ligand4.py -l "+str(ligand_pdb)+" -o "+str(ligand_pdbqt))
            print("Done.")
        except:
        ...
        #set files names
        f_in_name = os.path.join('vina_ligands', str("ligand_"+str()+".pdbqt"))
        recept_num = 1
